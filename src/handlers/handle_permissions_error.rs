...
pub async fn handle_permissions_error(
    _bot: &Bot,
    msg: &Message,
    _cmd: &dyn Command,
    ctx: &Context,
    _extras: &Extras,
    perm_error: PermissionErrors,
) {
    let _ = msg
        .channel_id
        .send_message(ctx, |m| {
            m.components(|c| {
                c.create_action_row(|row| {
                    row.create_button(|btn| {
                        btn.label("Dismiss")
                            .custom_id(format!("{}_{}", "dismiss", msg.author.id))
            ...
            m.embed(|embed| {
                ...
                embed
                    ...
                    .description({
                        match perm_error {
                            PermissionErrors::Staff => {
                                "Only staff members can execute this command."
                            }
