        ...
        .send_message(ctx, |m| {
            m.add_embed(|embed| {
                ...
                embed
                    ...
                    .description({
                        if current.is_empty() {
                            ...
                            format!("{} for argument `{}`.", err, arg.name.to_owned())
                        }
                    })
                    .footer(|ft| ft.text("Limitations..."))
                    .timestamp(iso)
                    .field(
                        "Expected",
                        {
                            match arg.expect {
                                RawArgTypes::Integer => "Integer",

                                RawArgTypes::Bool => "Boolean",

                                RawArgTypes::String => "String",
                            }
                        },
                        false,
                    )
                ...
                if !arg.example.is_empty() {
                    embed.field("Argument Example", arg.example.to_string(), false);
                };
